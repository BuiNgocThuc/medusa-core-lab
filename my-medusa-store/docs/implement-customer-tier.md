...
When you install a Medusa application, you get a fully-fledged commerce platform with a Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/resources/commerce-modules), which are available out-of-the-box. These features include customer and promotion management capabilities.
...
In Medusa, you can build custom features in a [module](https://docs.medusajs.com/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain. Medusa integrates the module into your application without affecting your setup.
...
Find all methods generated by the `MedusaService` in [the Service Factory reference](https://docs.medusajs.com/resources/service-factory-reference).
...
Medusa's CLI tool can generate the migrations for you. To generate a migration for the Tier Module, run the following command in the `apps/backend` directory:
...
When you defined the `Tier` data model, you added properties that store IDs of records managed by other modules. For example, the `promo_id` property stores a promotion ID, but promotions are managed by the [Promotion Module](https://docs.medusajs.com/resources/commerce-modules/promotion).
...
- A [workflow](https://docs.medusajs.com/learn/fundamentals/workflows) with steps to create a tier.
- An [API route](https://docs.medusajs.com/learn/fundamentals/api-routes) that exposes the workflow's functionality to client applications.
...
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the created tier with its rules.
...
```ts title="src/workflows/create-tier.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
...
export const createTierWorkflow = createWorkflow(
  ...
  (input: CreateTierWorkflowInput) => {
    ...
    const promoId = transform(
      { input },
      ({ input }) => input.promo_id || null
    )
...
- Use [when-then](https://docs.medusajs.com/learn/fundamentals/workflows/conditions) to check whether the promotion ID is provided and retrieve the promotion to validate that it exists.
  ...
  - This step uses [Query](https://docs.medusajs.com/learn/fundamentals/module-links/query) under the hood to retrieve data across modules.
- Create the tier using the `createTierStep`.
- Use [when-then](https://docs.medusajs.com/learn/fundamentals/workflows/conditions) to conditionally create tier rules if they're provided using the `createTierRulesStep`.
...
In workflows, you need `when-then` to check conditions based on execution values. Learn more in the [Conditions](https://docs.medusajs.com/learn/fundamentals/workflows/conditions) workflow documentation.
...
Refer to the [API routes](https://docs.medusajs.com/learn/fundamentals/api-routes) documentation to learn more about them.
...
To ensure incoming request bodies are validated, you need to apply a [middleware](https://docs.medusajs.com/learn/fundamentals/api-routes/middlewares).
...
Refer to the [Middlewares](https://docs.medusajs.com/learn/fundamentals/api-routes/middlewares) documentation to learn more.
...
Refer to the [Admin Development](https://docs.medusajs.com/learn/fundamentals/admin) documentation to learn more.

### a. Initialize JS SDK

To send requests to the Medusa server, you'll use the [JS SDK](https://docs.medusajs.com/resources/js-sdk). It's already installed in your Medusa project, but you need to initialize it before using it in your customizations.
...
Learn more about the initialization options in the [JS SDK](https://docs.medusajs.com/resources/js-sdk) reference.
...
Learn more about UI routes in the [UI Routes documentation](https://docs.medusajs.com/learn/fundamentals/admin/ui-routes).
...
- Create a DataTable instance from [Medusa UI](https://docs.medusajs.com/ui). You pass the columns, data, and pagination configurations to the hook.
...
Before creating a tier, you should [create a promotion](https://docs.medusajs.com/user-guide/promotions/create).
...
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the tier to update.
- [updateTierStep](#updateTierStep): Update the tier.
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the updated tier with rules.
...
```ts title="src/workflows/update-tier.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
...
In workflows, you need `transform` to prepare data based on execution values. Learn more in the [Data Manipulation](https://docs.medusajs.com/learn/fundamentals/workflows/variable-manipulation) workflow documentation.
...
Notice that you use the `query.index` method. This method is similar to `query.graph` but allows you to filter by linked records using the [Index Module](https://docs.medusajs.com/learn/fundamentals/module-links/index-module).
...
The [Index Module](https://docs.medusajs.com/learn/fundamentals/module-links/index-module) is a tool for performing high-performance queries across modules, such as filtering linked modules.
...
Refer to the [Index Module](https://docs.medusajs.com/learn/fundamentals/module-links/index-module) documentation to learn more.
...
- A [subscriber](https://docs.medusajs.com/learn/fundamentals/events-and-subscribers) that listens to the order placement event and executes the workflow.
...
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the details of the order with its customer and tier.
- [validateCustomerStep](#validateCustomerStep): Validate that the customer is a registered customer.
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the completed orders for the customer in the same currency.
...
```ts title="src/workflows/update-customer-tier-on-order.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
...
To test the workflow and subscriber, you'll need to place an order using the [Next.js Starter Storefront](https://docs.medusajs.com/resources/nextjs-starter) that you installed in the first step.
...
- [useQueryGraphStep](https://docs.medusajs.com/resources/references/helper-steps/useQueryGraphStep): Retrieve the details of the cart with its customer and tier.
- [acquireLockStep](https://docs.medusajs.com/resources/references/medusa-workflows/steps/acquireLockStep): Acquire a lock on the cart to prevent concurrent modifications.
- [releaseLockStep](https://docs.medusajs.com/resources/references/medusa-workflows/steps/releaseLockStep): Release the lock on the cart.
...
```ts title="src/workflows/add-tier-promotion-to-cart.ts"
import {
  createWorkflow,
  WorkflowResponse,
  transform,
  ...
  (input: AddTierPromotionToCartWorkflowInput) => {
    ...
    when({ validationResult, carts }, (data) => {
      ...
      const promoCodes = transform(
        { validationResult },
        ({ validationResult }) => [validationResult?.promotion_code!]
      )
...
When a customer adds a promotion to the cart, the storefront sends a request to the [Add Promotions API Route](https://docs.medusajs.com/api/store/carts/add-promotions), which executes the [updateCartPromotionsWorkflow](https://docs.medusajs.com/resources/references/medusa-workflows/updateCartPromotionsWorkflow).

To ensure that customers don't add promotions that belong to different tiers, you can consume the `validate` [hook](https://docs.medusajs.com/learn/fundamentals/workflows/workflow-hooks) of the `updateCartPromotionsWorkflow`. A hook is a specific point in a workflow where you can inject custom functionality.
...
```tsx title="src/modules/common/customer-tier/index.tsx"
...
```tsx title="src/modules/account/components/overview/index.tsx"
...
```tsx title="src/app/[countryCode]/(main)/account/@dashboard/page.tsx"
...
```tsx title="src/app/[countryCode]/(main)/order/[id]/confirmed/page.tsx"
...
- Automated emails to customers when they reach a new tier. Cloud users can benefit from zero-config email setup with [Medusa Emails](https://docs.medusajs.com/cloud/emails).
...
If you're new to Medusa, check out the [main documentation](https://docs.medusajs.com/learn) for a more in-depth understanding of the concepts you've used in this guide and more.

To learn more about the commerce features Medusa provides, check out [Commerce Modules](https://docs.medusajs.com/resources/commerce-modules).
