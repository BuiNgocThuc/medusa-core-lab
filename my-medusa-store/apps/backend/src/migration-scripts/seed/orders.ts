 ...
 */
async function getCustomerMap(customerModule: any, emails: string[]): Promise<Map<string, string>> {
 ...
 */
async function cleanUpExistingOrders(orderModule: any, customerIds: string[], logger: any) {
    const existing = await orderModule.listOrders({ customer_id: customerIds }, { select: ["id"] });

    if (existing.length) {
        await orderModule.deleteOrders(existing.map((o: any) => o.id));
        logger.info(`[seed:orders] removed ${existing.length} prior demo orders (idempotent).`);
    }
}
...
function buildSingleOrder(
    ...
    return {
        ...
        ...(context.salesChannelId ? { sales_channel_id: context.salesChannelId } : {}),
    };
}
...
export async function seedOrders({ container }: ExecArgs) {
    ...
    if (!custIdByEmail.size) {
        logger.warn("[seed:orders] no demo customers found — run customers.ts first. Aborting.");
        return;
    }
    ...
    logger.info(`[seed:orders] created ${orders.length} demo orders (${totalUnits} total units).`);
}
