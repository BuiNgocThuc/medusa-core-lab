 ...
 *   npx medusa exec ./src/scripts/customers.ts
 *
 * Credentials are LOCAL DEV FIXTURES, not secrets. Emails use the reserved
 * `.test`-style pattern below.
 *
...
export const DEMO_CUSTOMERS = [
    { email: "conghung@gmail.com", first_name: "Cong", last_name: "Hung" },
    { email: "ngocthuc@gmail.com", first_name: "Ngoc", last_name: "Thuc" },
    { email: "congson@gmail.com", first_name: "Cong", last_name: "Son" },
];
...
export async function seedCustomers({ container }: ExecArgs) {
    ...
    for (const c of DEMO_CUSTOMERS) {
        ...
        if (!reg.success || !reg.authIdentity) {
            logger.error(`[seed:customers] register failed for ${c.email}: ${reg.error}`);
            continue;
        }
