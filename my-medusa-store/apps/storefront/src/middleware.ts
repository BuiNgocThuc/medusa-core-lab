...
async function getRegionMap(cacheId: string) {
  ...
  if (
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[Middleware] Backend returned status ${response.status} when fetching regions.`)
      ...
      console.error(`[Middleware] Unable to connect to Medusa backend at ${BACKEND_URL}. Ensure backend is running.`, error)
    }
  }
