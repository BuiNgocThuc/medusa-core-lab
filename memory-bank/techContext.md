...
- **Medusa v2 Integration Test Environment Gotchas (Task 5):**
  - `jest.config.js` trong Medusa v2 bat buoc phai import `loadEnv` tu `@medusajs/framework/utils` thay vi `@medusajs/utils` (goi cu bi loai bo trong pnpm monorepo).
  - `medusaIntegrationTestRunner` ket noi PostgreSQL thong qua cac bien moi truong rieng le `DB_HOST`, `DB_PORT`, `DB_USERNAME`, `DB_PASSWORD` (doc tu process.env luc khoi tao `databaseCredentials`), khong phan tich truc tiep chuoi `DATABASE_URL`. Do do, bat buoc phai co file `.env.test` khai bao `DB_PORT=5434`, `DB_USERNAME=admin`, `DB_PASSWORD=123456` de trinh test ket noi dung container thay vi fallback ve cong 5432.
  - Remote Link ghi qua `link.create([{ [Modules.CUSTOMER]: { customer_id }, loyalty: { loyalty_account_id } }])`, va doc qua Query Graph `query.graph({ entity: "customer", fields: ["loyalty_account.*"] })` thong qua stored link table, hoan toan doc lap voi reference ID tren entity model.
